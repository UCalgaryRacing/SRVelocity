...
export default class DataBox extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            sensorData: SensorData.getInstance().getSensors()
        }
    }
    ...
    getColour = () => {
        ...
        let sensor = this.state.sensorData.filter(function(item) {return item.name === this.state.displayName});
        let yellowUpper = sensor.yellowUpper;
        let yellowLower = sensor.yellowLower;
        let redUpper = sensor.redUpper;
        let redLower = sensor.redLower;
