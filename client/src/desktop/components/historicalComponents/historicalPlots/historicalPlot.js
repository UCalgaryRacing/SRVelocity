...
export default class HistoricalPlotDash extends React.Component {
    ...
    updateOptions = () => {
        let temp = this.state.currentCSV.meta.fields.map( field => { return {value: field, label: field} })
        this.setState({
            options: temp
        })
    }
    ...
    handleYChange = (selectedOption) => {
        let tempData = this.state.currentCSV.data.map( dict => dict[selectedOption.value])
